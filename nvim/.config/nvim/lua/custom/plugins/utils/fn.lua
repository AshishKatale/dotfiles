...
M.toggle_list_chars = function()
  if not vim.g._usr_listchars then
...
M.toggle_format_on_save = function()
  if vim.g._usr_format_on_save_autocmd_id ~= nil then
    vim.api.nvim_del_autocmd(vim.g._usr_format_on_save_autocmd_id)
    vim.g._usr_format_on_save_autocmd_id = nil
    vim.notify('Disabled format on save.')
  else
    vim.g._usr_format_on_save_autocmd_id = vim.api.nvim_create_autocmd('BufWritePost', {
      callback = function()
...
M.scratch_buffer = function()
  if vim.g._usr_scratch and
      (vim.g._usr_scratch:win_valid() or vim.g._usr_scratch:buf_valid()) then
    vim.g._usr_scratch:toggle()
    return
  end

  vim.g._usr_scratch = require('snacks').win({
    width = 0.85,
    height = 0.85,
    border = 'rounded',
